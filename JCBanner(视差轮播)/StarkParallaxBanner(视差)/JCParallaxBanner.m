...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.scrollView && fmod(scrollView.contentOffset.x,self.frame.size.width) != 0) {
...
- (UIImageView *)imageViewOfCurrentCollectionCell{
    
    //  获取 parallaxCollectionView scrollView
    UIScrollView * scrollView = (UIScrollView *)self.parallaxCollectionView;
    
    //  获取当前 index
    NSInteger index =  scrollView.contentOffset.x/self.frame.size.width;
    
    //  获取当前 currentCell
    JCCollectionViewCell * currentCell = (JCCollectionViewCell*)[self.parallaxCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];

    return currentCell.parallaxImageView;
}
